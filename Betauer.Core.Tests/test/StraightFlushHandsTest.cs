...
public class StraightFlushHandsTest : PokerHandsTestBase {
    ...
    [Test]
    public void WithTenCards_ShouldFindMultipleStraightFlushes() {
        ...
        Assert.Multiple(() => {
            Assert.That(straightFlushes.Count, Is.GreaterThan(1),
                "Should find multiple straight flushes");
            ...
            var highestStraightFlush = straightFlushes
                .FirstOrDefault(sf => sf.Cards
                    .Select(c => c.Rank)
                    .OrderBy(r => r)
                    .SequenceEqual(new[] { 10, 11, 12, 13, 14 }));
            Assert.That(highestStraightFlush, Is.Not.Null,
                "Should find 10-A straight flush");

            // Verificar que encontramos una escalera baja (5-9)
            var lowestStraightFlush = straightFlushes
                .FirstOrDefault(sf => sf.Cards
                    .Select(c => c.Rank)
                    .OrderBy(r => r)
                    .SequenceEqual(new[] { 5, 6, 7, 8, 9 }));
            Assert.That(lowestStraightFlush, Is.Not.Null,
    ...
    public void SuggestDiscards_WithLimitedMaxDiscards() {
        ...
        Assert.Multiple(() => {
            ...
            Assert.That(discards[0].Select(c => c.Rank).OrderBy(r => r).ToList(),
                Is.EqualTo(new[] { 2, 3 }), "Should discard lowest club cards");
        });
    }

    [Test]
    public void SuggestDiscards_WithMultipleSuits() {
        // 7♥,8♥,9♥,J♥,Q♥,2♣,3♣,4♦,5♦ (cartas de tres palos diferentes)
