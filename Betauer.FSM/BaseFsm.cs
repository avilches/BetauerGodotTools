...
public abstract class BaseFsm<TStateKey, TEventKey, TState> : Fsm
    ...
    protected BaseFsm(TStateKey initialState, string? name = null) {
        Name = name;
    ...
    public void Send(TEventKey eventKey, int weight = 0) {
        if (!IsInitialized())
            throw new InvalidStateException("FSM not initialized. Please, execute it at least once before start sending events");
    ...
    protected TState FindState(TStateKey stateKey) {
        return States.TryGetValue(stateKey, out var state)
            ? state
            : throw new StateNotFoundException($"State {stateKey} not found. Please add it to the FSM");
    }
    ...
    private Command<TStateKey, TEventKey> _LocateCommand(TEventKey e) {
        ...
        throw new EventNotFoundException($"Event {e} not found. Please add it to the FSM");
    }
