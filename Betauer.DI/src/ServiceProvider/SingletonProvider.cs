...
public class SingletonProvider : Provider {
    ...
    public SingletonProvider(Type exposedType, Type instanceType, Func<object>? factory = null, string? name = null, bool lazy = false, Dictionary<string, object>? metadata = null) : base(exposedType, instanceType, name, metadata) {
        _factory = factory ?? CreateDefaultFactory(instanceType, Lifetime.Singleton);
        Lazy = lazy;
    }
