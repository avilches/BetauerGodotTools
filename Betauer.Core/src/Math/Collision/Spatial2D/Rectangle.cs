...
namespace Betauer.Core.Math.Collision.Spatial2D;
...
public class Rectangle : Shape {
    ...
    public override bool IntersectCircle(float x, float y, float radius) {
        return Geometry.Geometry.IntersectCircleRectangle(x, y, radius, Position.X, Position.Y, Width, Height);
    }

    public override bool IntersectRectangle(float x, float y, float width, float height) {
        return Geometry.Geometry.IntersectRectangles(Position.X, Position.Y, Width, Height, x, y, width, height);
    }

    public override bool IntersectPoint(float px, float py) {
        return Geometry.Geometry.IsPointInRectangle(px, py, this);
    }

    public override (int, int)[] GetIntersectingCells(float cellSize) {
        return SpatialGrid.GetRectangleIntersectingCells(Position.X, Position.Y, Width, Height, cellSize);
    }
