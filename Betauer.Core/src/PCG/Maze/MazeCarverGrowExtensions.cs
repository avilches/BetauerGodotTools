...
public static class MazeCarverGrowExtensions {
    /// <summary>
    /// Generates a maze starting from a specific cell, following the given constraints.
    /// </summary>
    /// <param name="carver">The MazeCarver instance</param>
    ...
    public static void Grow(this MazeCarver carver, Vector2I start, BacktrackConstraints constraints, Func<List<Vector2I>, Vector2I>? backtracker = null) {
        ArgumentNullException.ThrowIfNull(constraints);
        if (carver.IsCarved(start)) return;
        ...
        carver.CarveCell(start);
        carvedCells.Add(start);
        var currentCell = start;

        while (carvedCells.Count > 0) {
            var availableDirections = carver.GetAvailableDirections(currentCell);
            ...
            if (availableDirections.Count == 0 || cellsCarvedInCurrentPath >= maxCellsPerPath || totalCellsCarved == maxTotalCells) {
                ...
                if (carvedCells.Count > 0) {
                    ...
                    if (carver.GetAvailableDirections(currentCell).Count == 0) {
                        continue;
                    }
                }
            ...
            carver.CarveCell(intermediateCell);
            carver.CarveCell(nextCellToCarve);
    ...
    public static void GrowRandom(this MazeCarver carver, Vector2I start, int maxTotalCells = -1, Random? rng = null) {
