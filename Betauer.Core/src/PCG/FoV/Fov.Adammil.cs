...
public class FovAdammil : IFovScanner {
    public void Scan(Vector2I origin, int radius, Action<Vector2I> reveal, Func<Vector2I, bool> isOpaqueFunc) {
        reveal(origin);
        for (uint octant = 0; octant < 8; octant++) {
            _Scan(octant, origin, radius, 1, new Slope(1, 1), new Slope(0, 1), reveal, isOpaqueFunc);
        }
    }

    // represents the slope Y/X as a rational number
    public readonly struct Slope(uint y, uint x) {
    ...
    private void _Scan(uint octant, Vector2I origin, int radius, uint x, Slope top, Slope bottom, Action<Vector2I> reveal, Func<Vector2I, bool> isOpaqueFunc) {
        ...
        for (; x <= (uint)radius; x++) // (x <= (uint)radius) == (radius < 0 || x <= radius)
        {
            ...
            } else // top < 1
            {
                ...
                if (IsOpaque(x, topY, octant, origin)) // if the tile blocks light (i.e. is a wall)...
                {
                    ...
                    if (top.GreaterOrEqual(topY * 2 + 1, x * 2) && !IsOpaque(x, topY + 1, octant, origin)) topY++;
                } else // the tile doesn't block light
                {
                    ...
                    uint ax = x * 2; // center
                    if (IsOpaque(x + 1, topY + 1, octant, origin)) ax++; // use bottom-right if the tile above and right is a wall
            ...
            } else // bottom > 0
            {
                ...
                if (bottom.GreaterOrEqual(bottomY * 2 + 1, x * 2) && IsOpaque(x, bottomY, octant, origin) &&
                    !IsOpaque(x, bottomY + 1, octant, origin)) {
                    bottomY++;
                }
            }
            ...
            {
                if (radius < 0 || x * x + y * y < radiusSquared) // skip the tile if it's out of visual range
                {
                    var isOpaque = IsOpaque(x, y, octant, origin);
                    ...
                    if (x != radius) // but don't bother adjusting them if this is the last column anyway
                    {
                        if (isOpaque) {
                            ...
                            { // so adjust the bottom vector upward and continue processing it in the next column
                                ...
                                if (IsOpaque(x, y + 1, octant, origin)) nx--; // top left if the corner is not beveled
                                ...
                                { // created by adjusting the bottom is only valid if that's the case
                                    ...
                                    else _Scan(octant, origin, radius, x + 1, top, new Slope(ny, nx), reveal, isOpaqueFunc);
                            ...
                            if (wasOpaque > 0) // if we found a transition from opaque to clear, adjust the top vector downwards
                            {
                                ...
                                // NOTE: if you're using full symmetry and want more expansive walls (recommended), comment out the next line
                                if (IsOpaque(x + 1, y + 1, octant, origin)) nx++; // check the right of the opaque tile (y+1), not this one
        ...
        // NOTE: the code duplication between IsOpaque and SetVisible is for performance. don't refactor the octant
        // translation out unless you don't mind a 18% drop in speed
        bool IsOpaque(uint x, uint y, uint octant, Vector2I origin) {
            ...
            return isOpaqueFunc(new Vector2I((int)nx, (int)ny));
        }
        ...
        void SetVisible(uint x, uint y, uint octant, Vector2I origin) {
            ...
            reveal(new Vector2I((int)nx, (int)ny));
        }
    }
}