...
public static class Generators {
    ...
    public static void EdenGrowth(
        ...
        emptyNeighbors.UnionWith(region.GetVonNeumannPositions(start).Where(b => !region[b]));
        var pixelsAdded = 1;
        ...
        while (pixelsAdded < numPixels && (active.Count > 0 || emptyNeighbors.Count > 0)) {
            if (active.Count > 0) {
                ...
                foreach (var pos in region.GetVonNeumannPositions(cell).Where(b => !region[b])
                    ...
                    if (random.NextDouble() < branchingProbability) {
                        ...
                        foreach (var newEmpty in region.GetVonNeumannPositions(pos).Where(b => !region[b])) {
                ...
                // Añadir nuevos vecinos vacíos
                foreach (var newEmpty in region.GetVonNeumannPositions(emptyCell).Where(b => !region[b])) {
                    emptyNeighbors.Add(newEmpty);
                }
                pixelsAdded++;
            }
    ...
    private static bool HasFilledNeighbor(Array2D<bool> region, Vector2I pos) {
        return region.GetVonNeumannPositions(pos).Any(b => region[b]);
    }
    ...
    public static void PercolationCluster(
        ...
        candidates.UnionWith(region.GetVonNeumannPositions(start));
        ...
        while (pixelsAdded < numPixels && (candidates.Count > 0 || active.Count >= min)) {
            if (candidates.Count == 0) {
                ...
                candidates.UnionWith(active
                    .SelectMany(p => region.GetVonNeumannPositions(p).Where(b => !region[b]))
            ...
            var neighbors = region.GetVonNeumannPositions(cell).Count(b => region[b]);
            ...
            if (random.NextDouble() < prob) {
                ...
                candidates.UnionWith(region
                    .GetVonNeumannPositions(cell).Where(b => !region[b])
                    .Where(pos => !filled.Contains(pos)));
                ...
                if (hasNeighbor) {
                    ...
                    if (active.Count > min) {
                        ...
                        foreach (var point in active) {
                            if (region.GetVonNeumannPositions(point).All(b => region[b])) {
