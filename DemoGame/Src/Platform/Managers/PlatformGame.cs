...
public partial class PlatformGame : Control, IInjectable {
	private static readonly World2D NoWorld = new(); // A cached World2D to re-use

	[Inject] private ITransient<WorldPlatform> WorldPlatform { get; set; }
	...
	public WorldPlatform WorldScene { get; private set; } = null!;
	...
	private int ActivePlayers => WorldScene?.Players?.Count ?? 0;
	...
	private void ContinueLoad(SaveGame<MySaveGameMetadata> saveGame) {
		...
		if (consumer.Players.Count == 1) AllowAddingP2();
		else NoAddingP2();
	...
	public void InitializeWorld() {
		...
		WorldScene = WorldPlatform.Create();
		_subViewport1.AddChild(WorldScene);
		HudScene.Enable();
	}

	public PlatformPlayerNode CreatePlayer1(int joypad) {
	...
	public PlatformPlayerNode LoadPlayer1(int joypad, PlatformSaveGameConsumer consumer) {
		var playerMapping = JoypadPlayersMapping.AddPlayer().SetJoypadId(joypad);
		var player = WorldScene.LoadPlayer(playerMapping, consumer.Players[0], consumer.Inventories[0]);
		player.SetCamera(_camera1);
		return player;
	}

	public PlatformPlayerNode CreatePlayer2(int joypad) {
	...
	public void ManageSplitScreen() {
		...
		if (ActivePlayers == 1) {
			...
			var split = VisiblePlayers == 2;
			var shouldSplit = WorldScene.SplitScreen(Size, split);
