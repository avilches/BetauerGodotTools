...
namespace Veronenger.Platform.Character.Player;
...
public partial class PlatformPlayerNode : Node, IInjectable, INodeGameObject {
	...
	private void ConfigureCharacter() {
		...
		CharacterBody2D.SetCollisionNode(this);
		...
		Ready += () => PlatformCollisionLayerManager.PlayerConfigureCollisions(this);

		PlatformCollisionLayerManager.PlayerPickableArea(this, OnPick);
	...
	private void ConfigurePlayerHurtArea() {
		Ready += () => PlatformCollisionLayerManager.PlayerConfigureHurtArea(_hurtArea);
	...
	private void ConfigureAttackArea() {
		Ready += () => {
			PlatformCollisionLayerManager.PlayerConfigureAttackArea(_attackArea1);
			PlatformCollisionLayerManager.PlayerConfigureAttackArea(_attackArea2);
		};
	...
	private void ConfigureFsm() {
		...
		void Shoot() {
			...
			bullet.ShootFrom(weapon, CharacterBody2D.ToGlobal(bulletPosition), bulletDirection,
				PlatformCollisionLayerManager.PlayerConfigureBulletRaycast,
				collision => {
