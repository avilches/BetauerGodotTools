...
public partial class PlatformZombieNode : NpcNode, IInjectable {
	
	private static readonly SequenceAnimation RedFlash;

	static PlatformZombieNode() {
	...
	private Vector2 PlayerPos => ((PlatformPlayerNode)MainStateMachine.Game!.WorldScene.ClosestPlayer(Marker2D.GlobalPosition)).Marker2D.GlobalPosition;
	...
	private void ConfigureAi() {
		_zombieAi = PlatformMeleeAi.Create(Handler, new PlatformMeleeAi.Sensor(this, PlatformBody, LateralState, () => PlayerPos, () => _delta));
	...
	private void ConfigureCharacter() {
		...
		_lazyRaycastToPlayer = new LazyRaycast2D().Config(PlatformCollisionLayerManager.NpcConfigureCollisions);
		_attackArea.SetCollisionNode(this);
		_hurtArea.SetCollisionNode(this);

		Ready += () => {
			PlatformCollisionLayerManager.NpcConfigureCollisions(CharacterBody2D);
			PlatformCollisionLayerManager.NpcConfigureCollisions(FloorRaycast);
			PlatformCollisionLayerManager.NpcConfigureCollisions(FinishFloorRight);
			PlatformCollisionLayerManager.NpcConfigureCollisions(FinishFloorLeft);
			PlatformCollisionLayerManager.EnemyConfigureAttackArea(_attackArea);
			PlatformCollisionLayerManager.EnemyConfigureHurtArea(_hurtArea);
	...
	public void ConfigureFsm() {    
		...
		_fsm.State(ZombieState.Death)
			.Enter(() => {
				PlatformCollisionLayerManager.DisableAttackAndHurtAreas(_attackArea, _hurtArea);
