...
using Godot;
using Veronenger.Platform.Character.InputActions;
using Veronenger.Platform.Persistent;

namespace Veronenger.Platform.Character.Npc;

public class PlatformMeleeAi : FsmSync<PlatformMeleeAi.State, PlatformMeleeAi.Event>, ICharacterAi {
    ...
    public static ICharacterAi Create(ICharacterHandler handler, Sensor sensor) {
        if (handler is NpcController controller) return new PlatformMeleeAi(controller, sensor);
        if (handler is PlayerHandler) return DoNothingAI.Instance;
        throw new Exception($"Unknown handler: {handler.GetType()}");
    }

    public PlatformMeleeAi(NpcController controller, Sensor sensor) : base(State.Patrol, "ZombieIA") {
    ...
    public class Sensor {
        private readonly PlatformZombieNode _platformZombieNode;
        ...
        public Sensor(PlatformZombieNode platformZombieNode, KinematicPlatformMotion body, LateralState lateralState, Func<Vector2> playerGlobalPosition, Func<float> delta) {
            _platformZombieNode = platformZombieNode;
            _body = body;
        ...
        public Random Random => _platformZombieNode.Random;
        ...
        public bool IsHurt() => _platformZombieNode.IsState(ZombieState.Hurt);
        public bool IsAttacking() => _platformZombieNode.IsState(ZombieState.Attacking);
        public bool IsOnWall() => _body.IsOnWall() && _body.IsOnWallRight() == IsFacingRight;

        public bool IsFacingToPlayer() => _platformZombieNode.IsFacingToPlayer();
        public bool CanSeeThePlayer() => _platformZombieNode.CanSeeThePlayer();

        public float DistanceToPlayer() => _platformZombieNode.DistanceToPlayer();                                                            

        public bool IsFrontFloorFinishing() => IsFacingRight
            ? !_platformZombieNode.FinishFloorRight.IsColliding()
            : !_platformZombieNode.FinishFloorLeft.IsColliding();
        ...
        public NpcGameObject Status => _platformZombieNode.NpcGameObject;
    }
}