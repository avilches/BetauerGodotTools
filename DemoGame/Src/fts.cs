...
public class fts {
    ...
    public static float ballistic_range(float speed, float gravity, float initial_height) {
        ...
        float range = (speed * cos / gravity) *
                      (speed * sin + Mathf.Sqrt(speed * speed * sin * sin + 2 * gravity * initial_height));
        return range;
    }
    ...
    //
    // return (bool): true if a valid solution was found
    public static bool solve_ballistic_arc_lateral(Vector3 proj_pos, float lateral_speed, Vector3 target,
        Vector3 target_velocity, float max_height_offset, out Vector3 fire_velocity, out float gravity,
        out Vector3 impact_point) {
        // Handling these cases is up to your project's coding standards
        Debug.Assert(proj_pos != target && lateral_speed > 0,
            "fts.solve_ballistic_arc_lateral called with invalid data");
