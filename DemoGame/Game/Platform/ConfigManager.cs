...
/// <summary>
/// ConfigManager is needed to transform Config classes to text and vice versa. Some GameObject don't have a unique Config class, so they need to be persisted
/// as string when the SaveObject is saved and recover it again from text when the SaveObject is loaded.
/// </summary>
[Singleton]
public class ConfigManager : IInjectable {
    ...
    private readonly Dictionary<string, IDynamicConfig> _configs = new();
    private readonly Dictionary<IDynamicConfig, string> _reverseConfig = new();
