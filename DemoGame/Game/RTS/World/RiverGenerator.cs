...
public class RiverGeneratorGraph {
    ...
    public static void GenerateRivers(Graph<BiomeCell> points, XyDataGrid<BiomeCell> biomeCells, int numberOfPoints, float minDistance, Random random) {
        FindRiverStartPoints(points, biomeCells, numberOfPoints, minDistance).ForEach((cell) => {
            SimulateRiverFlow(points, biomeCells, cell, random);
        });
    }

    public static List<BiomeCell> FindRiverStartPoints(Graph<BiomeCell> graph, XyDataGrid<BiomeCell> biomeCells, int numberOfPoints, float minDistance) {
        var highestPoints = new List<BiomeCell>();
    ...
    public static void SimulateRiverFlow(Graph<BiomeCell> points, XyDataGrid<BiomeCell> biomeCells, BiomeCell startPoint, Random random) {
        var waterAmount = initialWater;
        var currentCell = startPoint;
        var river = new HashSet<BiomeCell>();
        var riverList = new List<BiomeCell>();
    ...
    public static void DrawRiver(List<BiomeCell> points, XyDataGrid<BiomeCell> biomeCells) {
        for (var i = 0; i < points.Count - 1; i++) {
            var start = points[i];
            var end = points[i + 1];
