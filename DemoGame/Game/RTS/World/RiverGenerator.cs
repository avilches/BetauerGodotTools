...
public class RiverGeneratorGraph {
    ...
    public static void GenerateRivers(Graph<BiomeCell> points, BiomeCell[,] biomeCells, int numberOfPoints, float minDistance, Random random) {
        FindRiverStartPoints(points, biomeCells, numberOfPoints, minDistance).ForEach((cell) => {
            SimulateRiverFlow(points, biomeCells, cell, random);
        });
    }

    public static List<BiomeCell> FindRiverStartPoints(Graph<BiomeCell> graph, BiomeCell[,] biomeCells, int numberOfPoints, float minDistance) {
        var highestPoints = new List<BiomeCell>();
    ...
    public static void SimulateRiverFlow(Graph<BiomeCell> points, BiomeCell[,] biomeCells, BiomeCell startPoint, Random random) {
        var waterAmount = initialWater;
        var currentCell = startPoint;
        var river = new HashSet<BiomeCell>();
        var riverList = new List<BiomeCell>();
