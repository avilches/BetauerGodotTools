...
namespace Betauer.GodotAction {
    public class HTTPRequestAction : HTTPRequest {
        ...
        public HTTPRequestAction OnRequestCompleted(Action<byte[], string[], int, int> action) {
            if (_onRequestCompletedAction == null || _onRequestCompletedAction.Count == 0) {
                _onRequestCompletedAction ??= new List<Action<byte[], string[], int, int>>(); 
                Connect("request_completed", this, nameof(_GodotSignalRequestCompleted));
            }
            _onRequestCompletedAction.Add(action);
            return this;
        }
        public HTTPRequestAction RemoveOnRequestCompleted(Action<byte[], string[], int, int> action) {
            if (_onRequestCompletedAction == null || _onRequestCompletedAction.Count == 0) return this;
            _onRequestCompletedAction.Remove(action); 
            if (_onRequestCompletedAction.Count == 0) {
                Disconnect("request_completed", this, nameof(_GodotSignalRequestCompleted));
            }
            return this;
        }
        private void _GodotSignalRequestCompleted(byte[] body, string[] headers, int response_code, int result) {
            if (_onRequestCompletedAction == null || _onRequestCompletedAction.Count == 0) return;
            for (var i = 0; i < _onRequestCompletedAction.Count; i++) _onRequestCompletedAction[i].Invoke(body, headers, response_code, result);
        }
