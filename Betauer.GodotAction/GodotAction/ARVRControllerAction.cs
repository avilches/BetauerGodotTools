    ...
    public class ARVRControllerAction : ARVRController {
        ...
        public ARVRControllerAction OnButtonPressed(Action<int> action) {
            if (_onButtonPressedAction == null || _onButtonPressedAction.Count == 0) {
                _onButtonPressedAction ??= new List<Action<int>>(); 
                Connect("button_pressed", this, nameof(_GodotSignalButtonPressed));
            }
        ...
        public ARVRControllerAction RemoveOnButtonPressed(Action<int> action) {
            ...
            if (_onButtonPressedAction.Count == 0) {
                Disconnect("button_pressed", this, nameof(_GodotSignalButtonPressed));
            }
            return this;
        }
        private void _GodotSignalButtonPressed(int button) {
            if (_onButtonPressedAction == null || _onButtonPressedAction.Count == 0) return;
            for (var i = 0; i < _onButtonPressedAction.Count; i++) _onButtonPressedAction[i].Invoke(button);
        }
        

        private List<Action<int>>? _onButtonReleaseAction; 
        public ARVRControllerAction OnButtonRelease(Action<int> action) {
            if (_onButtonReleaseAction == null || _onButtonReleaseAction.Count == 0) {
                _onButtonReleaseAction ??= new List<Action<int>>(); 
                Connect("button_release", this, nameof(_GodotSignalButtonRelease));
            }
            _onButtonReleaseAction.Add(action);
            return this;
        }
        public ARVRControllerAction RemoveOnButtonRelease(Action<int> action) {
            if (_onButtonReleaseAction == null || _onButtonReleaseAction.Count == 0) return this;
            _onButtonReleaseAction.Remove(action); 
            if (_onButtonReleaseAction.Count == 0) {
                Disconnect("button_release", this, nameof(_GodotSignalButtonRelease));
            }
            return this;
        }
        private void _GodotSignalButtonRelease(int button) {
            if (_onButtonReleaseAction == null || _onButtonReleaseAction.Count == 0) return;
            for (var i = 0; i < _onButtonReleaseAction.Count; i++) _onButtonReleaseAction[i].Invoke(button);
        }
        ...
        public ARVRControllerAction OnMeshUpdated(Action<Mesh> action) {
            if (_onMeshUpdatedAction == null || _onMeshUpdatedAction.Count == 0) {
                _onMeshUpdatedAction ??= new List<Action<Mesh>>(); 
                Connect("mesh_updated", this, nameof(_GodotSignalMeshUpdated));
            }
            _onMeshUpdatedAction.Add(action);
            return this;
        }
        public ARVRControllerAction RemoveOnMeshUpdated(Action<Mesh> action) {
            if (_onMeshUpdatedAction == null || _onMeshUpdatedAction.Count == 0) return this;
            _onMeshUpdatedAction.Remove(action); 
            if (_onMeshUpdatedAction.Count == 0) {
                Disconnect("mesh_updated", this, nameof(_GodotSignalMeshUpdated));
            }
            return this;
        }
        private void _GodotSignalMeshUpdated(Mesh mesh) {
            if (_onMeshUpdatedAction == null || _onMeshUpdatedAction.Count == 0) return;
            for (var i = 0; i < _onMeshUpdatedAction.Count; i++) _onMeshUpdatedAction[i].Invoke(mesh);
        }
