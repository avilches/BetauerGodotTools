...
namespace Betauer.GodotAction {
    public class Skeleton2DAction : Skeleton2D {
        ...
        public Skeleton2DAction OnBoneSetupChanged(Action action) {
            if (_onBoneSetupChangedAction == null || _onBoneSetupChangedAction.Count == 0) {
                _onBoneSetupChangedAction ??= new List<Action>(); 
                Connect("bone_setup_changed", this, nameof(_GodotSignalBoneSetupChanged));
            }
            _onBoneSetupChangedAction.Add(action);
            return this;
        }
        public Skeleton2DAction RemoveOnBoneSetupChanged(Action action) {
            if (_onBoneSetupChangedAction == null || _onBoneSetupChangedAction.Count == 0) return this;
            _onBoneSetupChangedAction.Remove(action); 
            if (_onBoneSetupChangedAction.Count == 0) {
                Disconnect("bone_setup_changed", this, nameof(_GodotSignalBoneSetupChanged));
            }
            return this;
        }
        private void _GodotSignalBoneSetupChanged() {
            if (_onBoneSetupChangedAction == null || _onBoneSetupChangedAction.Count == 0) return;
            for (var i = 0; i < _onBoneSetupChangedAction.Count; i++) _onBoneSetupChangedAction[i].Invoke();
        }
