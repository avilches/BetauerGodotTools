...
namespace Betauer.GodotAction {
    public class SkeletonAction : Skeleton {
        ...
        public SkeletonAction OnSkeletonUpdated(Action action) {
            if (_onSkeletonUpdatedAction == null || _onSkeletonUpdatedAction.Count == 0) {
                _onSkeletonUpdatedAction ??= new List<Action>(); 
                Connect("skeleton_updated", this, nameof(_GodotSignalSkeletonUpdated));
            }
            _onSkeletonUpdatedAction.Add(action);
            return this;
        }
        public SkeletonAction RemoveOnSkeletonUpdated(Action action) {
            if (_onSkeletonUpdatedAction == null || _onSkeletonUpdatedAction.Count == 0) return this;
            _onSkeletonUpdatedAction.Remove(action); 
            if (_onSkeletonUpdatedAction.Count == 0) {
                Disconnect("skeleton_updated", this, nameof(_GodotSignalSkeletonUpdated));
            }
            return this;
        }
        private void _GodotSignalSkeletonUpdated() {
            if (_onSkeletonUpdatedAction == null || _onSkeletonUpdatedAction.Count == 0) return;
            for (var i = 0; i < _onSkeletonUpdatedAction.Count; i++) _onSkeletonUpdatedAction[i].Invoke();
        }
