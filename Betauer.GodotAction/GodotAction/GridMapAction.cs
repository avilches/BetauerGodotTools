...
namespace Betauer.GodotAction {
    public class GridMapAction : Node {
        public GridMapAction() {
            SetProcess(false);
        ...
        public GridMapAction OnProcess(Action<float> action) {
        ...
        public GridMapAction RemoveOnProcess(Action<float> action) {
        ...
        public GridMapAction OnCellSizeChanged(Action<Vector3> action, bool oneShot = false, bool deferred = false) {
            if (_onCellSizeChangedAction == null || _onCellSizeChangedAction.Count == 0) {
                _onCellSizeChangedAction ??= new List<Action<Vector3>>(); 
                GetParent().Connect("cell_size_changed", this, nameof(_GodotSignalCellSizeChanged));
            }
            _onCellSizeChangedAction.Add(action);
            return this;
        }
        public GridMapAction RemoveOnCellSizeChanged(Action<Vector3> action) {
            if (_onCellSizeChangedAction == null || _onCellSizeChangedAction.Count == 0) return this;
            _onCellSizeChangedAction.Remove(action); 
            if (_onCellSizeChangedAction.Count == 0) {
                GetParent().Disconnect("cell_size_changed", this, nameof(_GodotSignalCellSizeChanged));
            }
            return this;
        }
        private void _GodotSignalCellSizeChanged(Vector3 cell_size) {
            if (_onCellSizeChangedAction == null || _onCellSizeChangedAction.Count == 0) return;
            for (var i = 0; i < _onCellSizeChangedAction.Count; i++) _onCellSizeChangedAction[i].Invoke(cell_size);
        }
