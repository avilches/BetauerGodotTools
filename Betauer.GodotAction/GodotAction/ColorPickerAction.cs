...
namespace Betauer.GodotAction {
    public class ColorPickerAction : Node {
        ...
        public ColorPickerAction OnPresetAdded(Action<Color> action, bool oneShot = false, bool deferred = false) {
            if (_onPresetAddedAction == null || _onPresetAddedAction.Count == 0) {
                _onPresetAddedAction ??= new List<Action<Color>>(); 
                GetParent().Connect("preset_added", this, nameof(_GodotSignalPresetAdded));
            }
            _onPresetAddedAction.Add(action);
            return this;
        }
        public ColorPickerAction RemoveOnPresetAdded(Action<Color> action) {
            if (_onPresetAddedAction == null || _onPresetAddedAction.Count == 0) return this;
            _onPresetAddedAction.Remove(action); 
            if (_onPresetAddedAction.Count == 0) {
                GetParent().Disconnect("preset_added", this, nameof(_GodotSignalPresetAdded));
            }
            return this;
        }
        private void _GodotSignalPresetAdded(Color color) {
            if (_onPresetAddedAction == null || _onPresetAddedAction.Count == 0) return;
            for (var i = 0; i < _onPresetAddedAction.Count; i++) _onPresetAddedAction[i].Invoke(color);
        }
        ...
        public ColorPickerAction OnPresetRemoved(Action<Color> action, bool oneShot = false, bool deferred = false) {
            if (_onPresetRemovedAction == null || _onPresetRemovedAction.Count == 0) {
                _onPresetRemovedAction ??= new List<Action<Color>>(); 
                GetParent().Connect("preset_removed", this, nameof(_GodotSignalPresetRemoved));
            }
            _onPresetRemovedAction.Add(action);
            return this;
        }
        public ColorPickerAction RemoveOnPresetRemoved(Action<Color> action) {
            if (_onPresetRemovedAction == null || _onPresetRemovedAction.Count == 0) return this;
            _onPresetRemovedAction.Remove(action); 
            if (_onPresetRemovedAction.Count == 0) {
                GetParent().Disconnect("preset_removed", this, nameof(_GodotSignalPresetRemoved));
            }
            return this;
        }
        private void _GodotSignalPresetRemoved(Color color) {
            if (_onPresetRemovedAction == null || _onPresetRemovedAction.Count == 0) return;
            for (var i = 0; i < _onPresetRemovedAction.Count; i++) _onPresetRemovedAction[i].Invoke(color);
        }
