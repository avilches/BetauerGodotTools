...
namespace Betauer.GodotAction {
    public class TileMapAction : TileMap {
        ...
        public TileMapAction OnSettingsChanged(Action action) {
            if (_onSettingsChangedAction == null || _onSettingsChangedAction.Count == 0) {
                _onSettingsChangedAction ??= new List<Action>(); 
                Connect("settings_changed", this, nameof(_GodotSignalSettingsChanged));
            }
            _onSettingsChangedAction.Add(action);
            return this;
        }
        public TileMapAction RemoveOnSettingsChanged(Action action) {
            if (_onSettingsChangedAction == null || _onSettingsChangedAction.Count == 0) return this;
            _onSettingsChangedAction.Remove(action); 
            if (_onSettingsChangedAction.Count == 0) {
                Disconnect("settings_changed", this, nameof(_GodotSignalSettingsChanged));
            }
            return this;
        }
        private void _GodotSignalSettingsChanged() {
            if (_onSettingsChangedAction == null || _onSettingsChangedAction.Count == 0) return;
            for (var i = 0; i < _onSettingsChangedAction.Count; i++) _onSettingsChangedAction[i].Invoke();
        }
