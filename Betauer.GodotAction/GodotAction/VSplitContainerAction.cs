...
namespace Betauer.GodotAction {
    public class VSplitContainerAction : Node {
        public VSplitContainerAction() {
            SetProcess(false);
        ...
        public VSplitContainerAction OnProcess(Action<float> action) {
            _onProcessActions ??= new List<Action<float>>(1);
            _onProcessActions.Add(action);
            SetProcess(true);
            return this;
        }
        public VSplitContainerAction OnPhysicsProcess(Action<float> action) {
        ...
        public VSplitContainerAction OnInput(Action<InputEvent> action) {
        ...
        public VSplitContainerAction OnUnhandledInput(Action<InputEvent> action) {
        ...
        public VSplitContainerAction OnUnhandledKeyInput(Action<InputEventKey> action) {
        ...
        public VSplitContainerAction RemoveOnProcess(Action<float> action) {
            _onProcessActions?.Remove(action);
            return this;
        }

        public VSplitContainerAction RemoveOnPhysicsProcess(Action<float> action) {
            _onPhysicsProcessActions?.Remove(action);
            return this;
        }

        public VSplitContainerAction RemoveOnInput(Action<InputEvent> action) {
            _onInputActions?.Remove(action);
            return this;
        }

        public VSplitContainerAction RemoveOnUnhandledInput(Action<InputEvent> action) {
            _onUnhandledInputActions?.Remove(action);
            return this;
        }

        public VSplitContainerAction RemoveOnUnhandledKeyInput(Action<InputEventKey> action) {
        ...
        public VSplitContainerAction OnDragged(Action<int> action, bool oneShot = false, bool deferred = false) {
            if (_onDraggedAction == null || _onDraggedAction.Count == 0) {
                _onDraggedAction ??= new List<Action<int>>(); 
                GetParent().Connect("dragged", this, nameof(_GodotSignalDragged));
            }
            _onDraggedAction.Add(action);
            return this;
        }
        public VSplitContainerAction RemoveOnDragged(Action<int> action) {
            if (_onDraggedAction == null || _onDraggedAction.Count == 0) return this;
            _onDraggedAction.Remove(action); 
            if (_onDraggedAction.Count == 0) {
                GetParent().Disconnect("dragged", this, nameof(_GodotSignalDragged));
            }
            return this;
        }
        private void _GodotSignalDragged(int offset) {
            if (_onDraggedAction == null || _onDraggedAction.Count == 0) return;
            for (var i = 0; i < _onDraggedAction.Count; i++) _onDraggedAction[i].Invoke(offset);
        }
